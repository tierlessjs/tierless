#!/usr/bin/env node
// npm create stackmix@latest my-app  →  a running two-tier Stackmix app in under a minute.
...
if (!target) { console.error("usage: npm create stackmix@latest <app-name>"); process.exit(2); }
const dest = path.resolve(target);
if (existsSync(dest) && readdirSync(dest).length) { console.error(`create-stackmix: ${target} exists and is not empty`); process.exit(2); }
...
Edit app.src.js and re-run; \`npx stackmix explain app.src.js\` shows what compiles and why.
`);
