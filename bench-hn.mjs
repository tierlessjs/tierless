// Waso — HN benchmark, single run (the concurrency x placement matrix).
//
...
// is on the server its per-level rounds are cheap server<->API hops. Waso wins
// even vs a hand-tuned parallel client, which still pays one RTT per tree level.

import { genThread, runMatrix, idSeq, DEFAULT_RTT, DEFAULT_API } from "./bench-core.mjs";
import { fmt } from "./waso-core.mjs";
...
const best = cells.wasoConc, naive = cells.restSeq, tuned = cells.restConc;
...
console.log(`   ${DEFAULT_RTT}ms RTT per tree level (${tuned.hops} levels); Waso pays 2 client RTTs total.)`);
