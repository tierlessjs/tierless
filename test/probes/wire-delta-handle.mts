...
import { isHandle, type Handle } from "tierless/graph";
...
// the fixed continuation shape this probe drives — data/alsoData start as the real dataset but arrive
// on the peer excised to a handle, so they're typed loosely; ui is the field this probe mutates directly.
interface BoardFrame extends DeltaFrame { data: unknown; alsoData: unknown; ui: { filter: string; page: number; sel: number } }
const mk = (): BoardFrame[] => [{ fn: "Board", pc: 1, data: bigData, alsoData: bigData, ui: { filter: "all", page: 0, sel: -1 } }];
...
const { stack: s1Raw } = applyDeltaTracked(B, c1.bytes);
const s1 = s1Raw as BoardFrame[];   // the round trip preserves this probe's own frame shape; the generic DeltaFrame[] return can't express that
...
// ---------------------------------------------------------------------------------------------
// s1[0].data is a handle here (confirmed by the isHandle check above; TS can't carry that across
// statements) and the fetched dataset's shape isn't declared anywhere but this probe's own fixture.
const fetched = tier.heapGet((s1[0].data as Handle).id) as any;
...
for (let hop = 0; hop < 10; hop++) {
  ...
  const { stack: sRaw } = applyDeltaTracked(B, c.bytes);
  const s = sRaw as BoardFrame[];   // ad hoc fixture — see note above
  if (c.shipped > 2) handleReshipped = true;                 // only the UI object should ship (+ maybe a frame)
  maxWarm = Math.max(maxWarm, c.bytes.length);
  if (!(isHandle(s[0].data) && s[0].data.id === (s1[0].data as Handle).id)) handleReshipped = true;
  hops++;
}
...
const { stack: onARaw } = applyDeltaTracked(A, back.bytes);
const onA = onARaw as BoardFrame[];   // ad hoc fixture — see note above
...
check("bounce: the dataset never left the server heap — deref on the way back still resolves it",
  (onA[0].data as Handle).id === (s1[0].data as Handle).id && (tier.heapGet((onA[0].data as Handle).id) as any).rows.length === 2000);
