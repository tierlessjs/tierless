...
const cases = [
  ...
  { file: "test/e2e/verify.mjs",         needs: ["PASS — auto-compiled tier-split continuation produced the correct session"] },
  { file: "test/e2e/conduit-verify.mjs", needs: ["the multi-view Conduit app (routing, forms, favorite, try/catch over a resource) runs correctly as one compiled continuation"] },
  ...
  { file: "test/e2e/api-verify.mjs",     needs: ["the api is an external reference monitor: authority is verified and enforced in a separate process on every call"] },
  { file: "test/e2e/api-pump.mjs",       needs: ["the live pump services api.* through the trusted monitor: a migrating continuation is authorized per principal in a separate process on every call"] },
  { file: "test/e2e/api-live.mjs",       needs: ["the default api.* path IS the reference monitor: the runtime pump serviced every api call over the pipe"] },
  ...
  { file: "test/e2e/control-flow.mjs",   needs: ["extended control flow survives migration"] },
  ...
  { file: "test/e2e/heap-writeback.mjs", needs: ["optimistic CAS, conflicts detected and retried, no lost updates"] },
  { file: "test/e2e/heap-write.mjs",     needs: ["transparent write-back: an ordinary member assignment propagated to the owning master"] },
  { file: "test/e2e/heap-write-delta.mjs", needs: ["a §5 write-back ships only the changed objects (member edits and collection mutations alike), far smaller than the whole snapshot, and min(delta, whole) is never larger"] },
  { file: "test/e2e/policy-live.mjs",    needs: ["informed FETCH", "priced migrate vs fetch from real bytes and steered the socket"] },
  { file: "test/e2e/delta-live.mjs",     needs: ["bounce + min(delta,full) + §5 excision + deref all compose, end to end"] },
];
