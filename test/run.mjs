...
const cases = [
  ...
  { file: "demos/verify.mjs",         needs: ["PASS — auto-compiled tier-split continuation produced the correct session"] },
  { file: "demos/conduit-verify.mjs", needs: ["the multi-view Conduit app (routing, forms, favorite, try/catch over a resource) runs correctly as one compiled continuation"] },
  ...
  { file: "demos/api-verify.mjs",     needs: ["the api is an external reference monitor: authority is verified and enforced in a separate process on every call"] },
  { file: "demos/api-pump.mjs",       needs: ["the live pump services api.* through the trusted monitor: a migrating continuation is authorized per principal in a separate process on every call"] },
  { file: "demos/api-live.mjs",       needs: ["the default api.* path IS the reference monitor: the runtime pump serviced every api call over the pipe"] },
  ...
  { file: "demos/control-flow.mjs",   needs: ["extended control flow survives migration"] },
  ...
  { file: "demos/heap-probe.mjs",     needs: ["big locals stay home, fetched on deref, single-writer coherent"] },
  { file: "demos/heap-live.mjs",      needs: ["the dataset stayed on the server, crossing only when the browser derefed it"] },
  { file: "demos/heap-auto.mjs",      needs: ["transparent deref: ordinary member access on a handle auto-fetches"] },
  { file: "demos/heap-writeback.mjs", needs: ["optimistic CAS, conflicts detected and retried, no lost updates"] },
  { file: "demos/heap-write.mjs",     needs: ["transparent write-back: an ordinary member assignment propagated to the owning master"] },
  { file: "demos/heap-write-delta.mjs", needs: ["a §5 write-back ships only the changed objects (member edits and collection mutations alike), far smaller than the whole snapshot, and min(delta, whole) is never larger"] },
  { file: "demos/policy-live.mjs",    needs: ["informed FETCH", "priced migrate vs fetch from real bytes and steered the socket"] },
  { file: "demos/delta-live.mjs",     needs: ["bounce + min(delta,full) + §5 excision + deref all compose, end to end"] },
];
