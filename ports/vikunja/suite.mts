...
// TIERLESS_EXEC_BATCH=1: burst coalescing ON (default off — measured neutral on time
// and bytes here, frames only; see COMPILING.md). Same build, one variable
// (packages/tierless/src/browser.mts reads the global).
html = html.replace(/<script data-tierless-batch>[^<]*<\/script>/, "");
if (process.env.TIERLESS_EXEC_BATCH) html = html.replace("<head>", "<head><script data-tierless-batch>window.__TIERLESS_EXEC_BATCH__=true;</script>");
