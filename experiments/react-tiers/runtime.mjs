...
import { PROGRAMS, __dispatch } from "./app/bundle.gen.mjs";
...
// Run a resource and route a failure into the continuation: if a try/catch is active
// (F.__h), jump to its catch/finally; otherwise the throw escapes this frame. This is
// what lets a resource that fails ON ANOTHER TIER be caught by a try/catch in the
// migrated code.
async function service(frame, req, execHere) {
  try { frame.ret = await execHere(req); }
  catch (err) {
    const target = __dispatch(frame, err);
    if (target == null) throw err;   // uncaught: propagate out of the continuation
    frame.pc = target;
  }
}

export async function pump(stack, ownsHere, execHere, incoming = null) {
  if (incoming) await service(stack[stack.length - 1], incoming, execHere);  // the resource that migrated us here
  for (;;) {
    ...
    if (r.op === "return") {
      ...
      throw r.value;                                    // uncaught in this frame (single-frame continuation)
    } else if (ownsHere(r.tier)) {
      await service(top, r, execHere);                  // owned resource: run it (routing any error), resume locally
    } else {
