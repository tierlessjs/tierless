...
//
// The continuation serializes to JSON (src/runtime/heap.mjs graph codec), crosses the
// socket via the project's wsPort/makePeer transport (src/runtime/wss.mjs), runs until
// it hits a resource the local tier doesn't own, and migrates to the owner. Render
// begins on the server and finishes in the browser the instant the vdom needs the real
// DOM — then bounces back to the server for the next api.* call. State lives in the
// continuation's frame locals, pinned to neither tier.
//
// Run:  node experiments/react-tiers/demo.mjs        (needs Playwright Chromium)
import { createRequire } from "node:module";
import { wsPort, makePeer } from "../../src/runtime/wss.mjs";
