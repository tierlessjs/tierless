...
| `test/probes/lang-coverage.mjs` | ordinary binding forms compile and migrate: `for`/`of`/`in`, destructuring (object/array/nested/default/rest, non-array iterables via an `Array.from` guard), and default/destructured/rest parameters — each driven across a JSON round-trip of the continuation at every suspension. The forms that genuinely can't migrate — a tier call inside a callback/comparator/method, run synchronously by native code that can't suspend — are rejected with a clear compile error, not silently miscompiled |
