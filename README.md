...
| `src/heap-write-delta.mjs` | a write-back IS a delta to the master: it ships only the objects that changed in the snapshot — member edits *and* collection mutations (push/`Map.set`/`Set.add`) — applied in place under CAS, `min(delta, whole)` so it's never larger (member edits cross **96×** smaller than the whole 1500-row snapshot) |
