// Stackmix — the Node host, assembled. Two calls cover the deployment shapes:
//
//   attachStackmix(httpServer, opts)   mount the session endpoint on an EXISTING http
...
export function attachStackmix(httpServer, { bundle, tier = "server", session, path: wsPath = WS_PATH }) {
...
export async function serveApp({ port = 0, page, staticRoot, ...attachOpts }) {
  ...
  const attached = attachStackmix(server, attachOpts);
