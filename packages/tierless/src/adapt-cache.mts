...
const freshLifetime = (headers: Record<string, string> | undefined): number => {
  const cc = (headers?.["cache-control"] ?? "").toLowerCase();
  if (CC_NO_STORE.test(cc)) return 0;
  const m = CC_MAX_AGE.exec(cc);
  if (m) return Number(m[1]) * 1000;
  if (cc) return 0;                                        // a directive that says nothing about age: no freshness
...
const varyValues = (vary: string | undefined, reqHeaders: Record<string, string> | undefined): string | null => {
  if (!vary) return "";
...
interface FreshEntry { exp: number; env: unknown; vary?: string; values: string; bytes: number }
...
const envelopeBytes = (env: unknown): number => {
  const raw = (env as Record<symbol, unknown>)[RAW_TEXT];
  if (typeof raw === "string") return raw.length;
  try { return JSON.stringify((env as { body?: unknown })?.body ?? "").length; } catch { return MEM_BUDGET_BYTES + 1; }   // unserializable: refuse to cache it
};
...
export function conditionalCrossings({ store, fence }: { store?: EnvelopeStore; fence?: Fence } = {}): { wrap(inner: Exec): Exec } {
  ...
  const memo = new Map<string, FreshEntry>();
  let memoBytes = 0;
  const memoDrop = (k: string): void => { const e = memo.get(k); if (e) { memoBytes -= e.bytes; memo.delete(k); } };
  const memoPut = (path: string, env: unknown, headers: Record<string, string> | undefined, reqHeaders: Record<string, string> | undefined, bytes: number): void => {
    const life = freshLifetime(headers);
    if (!life) return;
    const values = varyValues(headers?.vary, reqHeaders);
    if (values === null || bytes > MEM_BUDGET_BYTES) return;
    memoDrop(path);
    memo.set(path, { exp: Date.now() + life, env, vary: headers?.vary, values, bytes });
    memoBytes += bytes;
    for (const k of memo.keys()) { if (memoBytes <= MEM_BUDGET_BYTES) break; memoDrop(k); dbg("fresh:evict", k); }
  ...
  return {
    wrap: (inner) => async (req) => {
      ...
      const reqHeaders = ((r.args ?? [])[2] as { headers?: Record<string, string> } | undefined)?.headers;
      ...
      if (hit) {
        if (hit.exp > Date.now() && hit.values === varyValues(hit.vary, reqHeaders)) {
          memo.delete(path); memo.set(path, hit);          // LRU touch
          dbg("fresh:hit", path);
          presentAs(r, hit.env);
          return hit.env;
        }
      ...
      if (!etag) {
        ...
        if (env?.status === 200 && r.name === "api.get") memoPut(path, env, env.headers, reqHeaders, envelopeBytes(env));
        return env;
      }
      ...
      if (env?.status !== 304) {
        ...
        if (env?.status === 200) memoPut(path, env, env.headers, reqHeaders, envelopeBytes(env));
        presentAs(r, env);
        return env;
      }
