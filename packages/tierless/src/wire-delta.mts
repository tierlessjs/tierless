...
// The mutable per-tier delta state: stable ids (idOf/store) and the §5 excision map (handleOf) are
// always present; the rest is mode-specific. makeDeltaSession (RESCAN) sets peerVer; makeTrackedSession
// (WRITE-TRACKED) sets seen/dirty; adoptBaseline sets based (and, for a tracked session, seen).
// openSnapshot runs both constructors' setup on one session (a write-back needs a tracked baseline
// AND peerVer), so the mode-specific fields are optional here rather than split across interfaces.
...
const ownKeys = (v: object): string[] => Object.keys(v).filter((k) => k !== "__proto__");
const isVarInt = (v: number): boolean => Number.isInteger(v) && Math.abs(v) < 0x80000000 && !Object.is(v, -0);
function fnv(s: string): number { let h = 0x811c9dc5 >>> 0; for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 0x01000193) >>> 0; } return h >>> 0; }
...
function forEachChild(v: unknown, fn: (child: unknown) => void): void {
  ...
  else for (const k of ownKeys(v as object)) fn((v as Record<string, unknown>)[k]);
}

type Slots = Map<string | number, { key: unknown; canon: string }>;
...
function canonOf(idOf: WeakMap<object, string>, sub: (v: unknown) => unknown, v: unknown): string {
  v = sub(v);
...
function slotsOf(idOf: WeakMap<object, string>, sub: (v: unknown) => unknown, v: unknown): Slots {
  const m: Slots = new Map();
  ...
  else for (const k of ownKeys(v as object)) m.set(k, { key: k, canon: canonOf(idOf, sub, (v as Record<string, unknown>)[k]) });
  return m;
}
...
class W {
  ...
  ensure(k: number): void { if (this.n + k > this.buf.length) { let c = this.buf.length * 2; while (c < this.n + k) c *= 2; const b = new Uint8Array(c); b.set(this.buf.subarray(0, this.n)); this.buf = b; } }
  u8(b: number): void { this.ensure(1); this.buf[this.n++] = b & 0xff; }
  raw(b: Uint8Array): void { this.ensure(b.length); this.buf.set(b, this.n); this.n += b.length; }
  varu(x: number): void { this.ensure(10); let v = x; for (;;) { const b = v % 128; v = Math.floor(v / 128); if (v !== 0) this.buf[this.n++] = b | 0x80; else { this.buf[this.n++] = b; break; } } }
  vari(x: number): void { this.varu(x >= 0 ? x * 2 : -x * 2 - 1); }
...
class R {
  ...
  len: number;
  dv: DataView;
  constructor(b: Uint8Array) { this.buf = b; this.n = 0; this.len = b.length; this.dv = new DataView(b.buffer, b.byteOffset, b.byteLength); }
  need(k: number): void { if (this.n + k > this.len) throw new RangeError("wire-delta: truncated"); }
  ...
  varu(): number { let v = 0, sh = 1, b: number, i = 0; do { this.need(1); b = this.buf[this.n++]; v += (b & 0x7f) * sh; sh *= 128; if (++i > 9) throw new RangeError("wire-delta: varint too long"); } while (b & 0x80); return v; }
  count(): number { const c = this.varu(); if (c > this.len - this.n) throw new RangeError("wire-delta: bad count"); return c; }
  vari(): number { const z = this.varu(); return z % 2 === 0 ? z / 2 : -(z + 1) / 2; }
  f64(): number { this.need(8); const v = this.dv.getFloat64(this.n, true); this.n += 8; return v; }
}

// RootFrame/RootReq are rootsOf's internal encoding of a stack frame / request as flat root-value
// ranges (b0/a0 + count) — the shape both emit() (writing the wire) and parseDelta() (reading it back)
// share, never exposed outside this module.
interface RootFrame {
  fn: string;
  pc: number;
  keys: string[];
...
function rootsOf(stack: DeltaFrame[], request: DeltaRequest | null): { rootVals: unknown[]; frames: RootFrame[]; req: RootReq | null } {
...
const sidOfFn = (session: Session) => (v: object): string => { let id = session.idOf.get(v); if (id === undefined) { id = session.tier + "#" + (session.next++); session.idOf.set(v, id); session.store.set(id, v); } return id; };
...
const subFn = (session: Session) => { const h = session.handleOf; return (v: unknown): unknown => (isObj(v) && h.has(v) ? h.get(v) : v); };
...
function exciseBig(session: Session, rootVals: unknown[], tier: EncodeTier, threshold: number, content?: { store: ContentStoreView } | null): void {
  const handleOf = session.handleOf, seen = new Set<object>();
  const walk = (v: unknown): void => {
    ...
    if (approxExceeds(v, threshold)) { handleOf.set(v, { __tierless_handle__: true, owner: tier.id, id: tier.heapPut(v), kind: Array.isArray(v) ? "array" : "object" }); return; }  // excise; don't descend
    forEachChild(v, walk);
  };
...
function orderPreserved(base: Slots, cur: Slots): boolean {
...
function planRecord(session: Session, sub: (v: unknown) => unknown, id: string, full = false): any {
  const v: any = session.store.get(id);
  ...
  let cur: any;
  if (useFields) { cur = slotsOf(session.idOf, sub, v); session.peerSlots.set(id, cur); }
...
function emit(session: Session, rootVals: unknown[], frames: RootFrame[], req: RootReq | null, changed: string[], full = false): Uint8Array {
  const sub = subFn(session);
  const records: any[] = changed.map((id) => planRecord(session, sub, id, full));
  const strMap = new Map<string, number>(), strs: string[] = [];
  const si = (s: string): number => { let i = strMap.get(s); if (i === undefined) { i = strs.length; strMap.set(s, i); strs.push(s); } return i; };
  const internVal = (v: unknown): void => { v = sub(v); if (isObj(v)) si(session.idOf.get(v)!); else if (typeof v === "string") si(v); else if (typeof v === "bigint") si(String(v)); };
  ...
  const w = new W();
  const node = (v: unknown): void => {                                  // a value: ref(sid) | int | float | str | bool | null | undef | bigint
    v = sub(v);
    ...
    else if (isVarInt(v as number)) { w.u8(1); w.vari(v as number); }
    else { w.u8(2); w.f64(v as number); }
  };
...
function parseDelta(bytes: Uint8Array | ArrayBufferLike): { frames: RootFrame[]; req: RootReq | null; rootNodes: any[]; changed: any[] } {
  ...
  const strs: string[] = []; { const n = r.count(); for (let i = 0; i < n; i++) strs.push(td.decode(r.raw(r.count()))); }
  const S = (i: number): string => { if (i < 0 || i >= strs.length) throw new RangeError("wire-delta: string index out of range"); return strs[i]; };
  ...
  const node = (): any => { const t = r.u8(); switch (t) {
  ...
  const changed: any[] = []; { const n = r.count(); for (let i = 0; i < n; i++) {
    ...
    else if (kind === 6) { const len = r.varu(); const sc = r.count(), sets: [number, any][] = []; for (let j = 0; j < sc; j++) { const idx = r.varu(); sets.push([idx, node()]); } changed.push({ sid, kind, len, sets }); }   // array PATCH
    else if (kind === 7) { const sc = r.count(), sets: [any, any][] = []; for (let j = 0; j < sc; j++) { const k = node(); sets.push([k, node()]); } const dc = r.count(), dels: any[] = []; for (let j = 0; j < dc; j++) dels.push(node()); changed.push({ sid, kind, sets, dels }); } // Map PATCH
    else if (kind === 8) { const ac = r.count(), adds: any[] = []; for (let j = 0; j < ac; j++) adds.push(node()); const dc = r.count(), dels: any[] = []; for (let j = 0; j < dc; j++) dels.push(node()); changed.push({ sid, kind, adds, dels }); }      // Set PATCH
...
function reconstruct(session: Session, parsed: ReturnType<typeof parseDelta>): { stack: DeltaFrame[]; request: DeltaRequest | null; rootVals: unknown[]; shipped: string[] } {
  const { frames, req, rootNodes, changed } = parsed;
  const shell = (k: number, h: any): any => (k === 1 || k === 6 ? [] : k === 2 ? h : k === 3 || k === 7 ? new Map() : k === 4 || k === 8 ? new Set() : {});
  for (const ch of changed) {                                            // pass 1: locate-or-create shells
    let obj: any = session.store.get(ch.sid);
    if (!obj) { obj = shell(ch.kind, ch.handle); session.store.set(ch.sid, obj); session.idOf.set(obj, ch.sid); }
    ch.obj = obj;
  }
  const deref = (nd: any): unknown => ("ref" in nd ? session.store.get(nd.ref) : nd.v);  // store has every referenced id
  ...
  const stack: DeltaFrame[] = frames.map((f) => { const fr: DeltaFrame = { fn: f.fn, pc: f.pc }; f.keys.forEach((k, i) => { fr[k] = rootVals[f.b0 + i]; }); return fr; });
  const request: DeltaRequest | null = req ? { op: req.op, tier: req.tier, name: req.name, args: rootVals.slice(req.a0, req.a0 + req.argc) } : null;
  return { stack, request, rootVals, shipped: changed.map((c) => c.sid) };
}

// ============================== RESCAN mode (no cooperation needed) ==============================

export function makeDeltaSession(tier: string): Session {
...
function scan(session: Session, rootVals: unknown[]): { reach: Map<string, object>; ver: Map<string, number> } {
  const sidOf = sidOfFn(session), sub = subFn(session);
  const reach = new Map<string, object>();
  const visit = (v: unknown): void => { v = sub(v); if (!isObj(v)) return; const id = sidOf(v); if (reach.has(id)) return; reach.set(id, v); forEachChild(v, visit); };
      ...
      : isMap(v) ? "M|" + [...v].map(([k, val]) => canon(k) + "=" + canon(val)).join("|")
        : isSet(v) ? "S|" + [...v].map(canon).join("|")
          : Array.isArray(v) ? "a|" + v.map(canon).join("|")
            : "o|" + ownKeys(v).map((k) => k + "=" + canon((v as Record<string, unknown>)[k])).join("|");
    ver.set(id, fnv(c));
  }
  return { reach, ver };
}
...
export function encodeDelta(session: Session, stack: DeltaFrame[], request: DeltaRequest | null, opts: DeltaOpts = {}): { bytes: Uint8Array; reachable: number; shipped: number } {
...
export function applyDelta(session: Session, bytes: Uint8Array | ArrayBufferLike): { stack: DeltaFrame[]; request: DeltaRequest | null } {
...
export function makeTrackedSession(tier: string): Session {
...
export function touch<T extends unknown[]>(session: Session, ...objs: T): T[0] {
...
function selectDirty(session: Session, rootVals: unknown[]): { changed: string[]; fresh: string[]; visited: number } {
  const sidOf = sidOfFn(session), sub = subFn(session);
  const ship = new Set<string>(), fresh: string[] = [];
  let visited = 0;
...
function selectDirtyExact(session: Session, rootVals: unknown[]): { changed: string[]; fresh: string[]; visited: number } {
  const sidOf = sidOfFn(session), sub = subFn(session);
  const reach: [string, object][] = [], seen = new Set<string>();
  const visit = (v: unknown): void => { v = sub(v); if (!isObj(v)) return; const id = sidOf(v); if (seen.has(id)) return; seen.add(id); reach.push([id, v]); forEachChild(v, visit); };
  rootVals.forEach(visit);
  const ship: string[] = [], fresh: string[] = [];
...
export function planDelta(session: Session, stack: DeltaFrame[], request: DeltaRequest | null, opts: PlanDeltaOpts = {}): { bytes: Uint8Array; shipped: number; visited: number; commit(): void } {
  const { rootVals, frames, req } = rootsOf(stack, request);
  if (opts.tier) exciseBig(session, rootVals, opts.tier, opts.threshold || 64 * 1024);
...
export function encodeDeltaTracked(session: Session, stack: DeltaFrame[], request: DeltaRequest | null, opts: PlanDeltaOpts = {}): { bytes: Uint8Array; shipped: number; visited: number } {
...
export function applyDeltaTracked(session: Session, bytes: Uint8Array | ArrayBufferLike): { stack: DeltaFrame[]; request: DeltaRequest | null } {
...
export function adoptBaseline(session: Session, stack: DeltaFrame[], request: DeltaRequest | null): void {
...
export function subForFullWire(session: Session, stack: DeltaFrame[], request: DeltaRequest | null, content: { store: ContentStoreView } | null = null): { stack: DeltaFrame[]; request: DeltaRequest | null } {
  const sub = subFn(session), memo = new Map<object, unknown>();
  const rebuild = (v: unknown): any => {
    v = sub(v);
    ...
    if (Array.isArray(v)) { const a: any[] = []; memo.set(v, a); for (const e of v) a.push(rebuild(e)); return a; }
    if (isMap(v)) { const m = new Map<unknown, unknown>(); memo.set(v, m); for (const [k, val] of v) m.set(rebuild(k), rebuild(val)); return m; }
    if (isSet(v)) { const s = new Set<unknown>(); memo.set(v, s); for (const e of v) s.add(rebuild(e)); return s; }
    const o: Record<string, unknown> = {}; memo.set(v, o); for (const k of ownKeys(v)) o[k] = rebuild((v as Record<string, unknown>)[k]); return o;
  };
  const stk: DeltaFrame[] = stack.map((f) => { const g: Record<string, unknown> = {}; for (const k of Object.keys(f)) g[k] = (k === "fn" || k === "pc") ? f[k] : rebuild(f[k]); return g as DeltaFrame; });
  const req: DeltaRequest | null = request ? { ...request, args: (request.args || []).map(rebuild) } : null;
...
export function exciseForCapture(session: Session, stack: DeltaFrame[], request: DeltaRequest | null, tier: EncodeTier, threshold: number = 64 * 1024, content: { store: ContentStoreView } | null = null): void {
...
const snapStack = (v: unknown): DeltaFrame[] => [{ fn: "@snap", pc: 0, v }];
...
export function openSnapshot(tierId: string, value: unknown): Session {
...
export function wholeSnapshot(session: Session, value: unknown): Uint8Array {
...
export function applySnapshot(tierId: string, master: unknown, bytes: Uint8Array | ArrayBufferLike): unknown {
  const session = makeTrackedSession(tierId);
