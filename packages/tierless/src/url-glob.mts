...
export type ForceBrowserDescriptor = { glob: string } | { re: [string, string] };
...
export function matchesForceBrowser(list: readonly ForceBrowserDescriptor[] | undefined, url: string): boolean {
...
const absGlob = (g: string): string =>
  /^[a-z]+:\/\//i.test(g) || g.startsWith("*") ? g : "**" + (g.startsWith("/") ? g : "/" + g);
