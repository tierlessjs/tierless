...
export function makeHost({ bundle, tier, exec, owns, meta = {}, trace, coherence: coherenceIn, twins }) {
    ...
    async function step(peer, sid, stack, incoming) {
        ...
        const twinDeltas = [];
        const sink = twins ? { twinDelta: (d) => twinDeltas.push(d) } : undefined;
        const carry = () => (twinDeltas.length ? { twinDeltas } : {});
        try {
            const res = await runPump(peer, stack, incoming, sink);
            if (res.done)
                return { obj: { type: "done", value: res.value, ...carry() } };
...
export function batchExec(peer) {
    ...
    const flush = () => {
        ...
        for (const b of batches) {
            if (b.waiters.length === 1) {
                const w = b.waiters[0];
                peer.request(b.payload, w.bin).then(w.res, w.rej);
                continue;
            }
            ...
            peer.request({ ...b.payload, type: "execBatch" }, encodeArgs(b.waiters.map((w) => decodeArgs(w.bin))))
                ...
                const r = obj.value?.[i];
                if (!r)
                    w.res({ obj: { type: "error", message: obj.message || "tierless: execBatch reply missing element " + i }, bin: null });
                else if (r.ok)
                    w.res({ obj: { type: "done", value: r.value }, bin: null });
                else
                    w.res({ obj: { type: "error", message: r.message, ...(r.response ? { response: r.response } : {}) }, bin: null });
            }), (e) => b.waiters.forEach((w) => w.rej(e)));
        }
    };
    return {
        request(payload, bin) {
            if (!payload || payload.type !== "exec" || !bin)
                return peer.request(payload, bin);
            return new Promise((res, rej) => {
                const key = JSON.stringify([payload.tier, payload.module ?? null]);
                let q = queues.get(key);
                if (!q)
                    queues.set(key, (q = { payload, waiters: [] }));
