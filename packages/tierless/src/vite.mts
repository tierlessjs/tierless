...
const resolveEmitTarget = (srcDir: string, spec: string): string => {
  const abs = path.resolve(srcDir, spec);
  if (existsSync(abs) && path.extname(abs)) return abs;
  for (const ext of [".mjs", ".js"]) if (existsSync(abs + ext)) return abs + ext;
  for (const ext of [".mts", ".ts", ".tsx", ".vue"]) if (existsSync(abs + ext)) {
...
const rewriteRelativeImports = (code: string, srcDir: string, outDir: string, specs: string[], mixByPath: Map<string, string>, aliases: Record<string, string> = {}): string => {
  let out = code;
  ...
  for (const find of Object.keys(aliases)) {
    ...
    if (hit) throw new Error(`tierless: a "use tierless" module imports through the Vite alias "${find}" — the emitted server bundle is not alias-aware; use a relative import to a .mjs/.js helper`);
  }
  return out;
};
...
export interface TierlessPlugin {
  ...
  configResolved(config: { root?: string; resolve?: { alias?: Array<{ find: string | RegExp; replacement: string }> }; build?: { ssr?: boolean | string } }): void;
  buildStart(): void;
  writeBundle(): void;
  resolveId(id: string): string | undefined;
  load(id: string): string | undefined;
  transformIndexHtml: { order: "pre"; handler(html: string): unknown };
...
export default function tierless(opts: TierlessPluginOptions = {}): TierlessPlugin {
  ...
  return {
    ...
    buildStart(): void { if (!ssrBuild) { machines.clear(); appMachines.clear(); coverage.clear(); } },   // fresh CLIENT build: transform repopulates the maps (an SSR build must not wipe them)
    ...
    writeBundle(): void {
      if (ssrBuild) return;   // the server (nitro/SSR) build: the artifacts below are the client build's
      ...
      if (!machines.size && !appMachines.size) {
        ...
        try { rmSync(path.resolve(root, serverOutDir, "tierless.manifest.json"), { force: true }); } catch { /* nothing stale */ }
        return;
      }
      const outDir = path.resolve(root, serverOutDir);
      mkdirSync(outDir, { recursive: true });
      const modules: Record<string, string> = {};
      const mixByPath = new Map<string, string>();   // resolved source path -> its server bundle filename
      for (const id of machines.keys()) mixByPath.set(path.resolve(id), serverBundleName(id));
