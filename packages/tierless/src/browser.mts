...
export function connect({ url, protocols, exec, bundle, tier = "browser", heap = true,
  ...
  const base = (globalThis as { __TIERLESS_EXEC_BATCH__?: boolean }).__TIERLESS_EXEC_BATCH__ ? batchExec(raw) : raw;
  ...
  const ready: Promise<void> = new Promise((res, rej) => {
    ...
    onEvent(ws, "error", (e: any) => fail("error" + (e && e.message ? ": " + e.message : "")));
    onEvent(ws, "close", () => fail(opened ? "closed" : "closed before it opened"));
  });
  void ready.catch(() => { /* the degrade path reads downReason(); an unawaited preconnect must not report unhandled */ });
  ...
  const peer: typeof raw = { ...base, request: async (payload, bin) => { await ready; return base.request(payload, bin); } };
  ...
  const register = (module: string, b: Bundle): Host => {
    const id = module || "";
    if (!hosts.has(id)) hosts.set(id, makeHost({ bundle: b, tier, exec: exec as Exec, meta: id ? { module: id } : {}, coherence, ...(appTrace ? { trace: appTrace as never } : {}) }));   // exec is optional here (actions-only pages never own a resource); makeHost only calls it when one is
    return hosts.get(id)!;
  };
  ...
  return {
    ...
    exec: async (req: import("./types.mjs").ResourceRequest): Promise<unknown> => {
      ...
      try {
        ...
        const r = (err as { response?: { status?: number; data?: unknown; headers?: Record<string, string> } } | null)?.response;
        record(r && typeof r.status === "number" ? r.status : undefined, r?.data, !!r && "data" in r, r?.headers);
        throw err;
      }
      const env = value as { status?: number; body?: unknown; headers?: Record<string, string> } | null;
      record(env && typeof env.status === "number" ? env.status : undefined, env?.body, !!env && "body" in (env as object), env?.headers);
      return value;
    },
    close: () => ws.close(),
  };
...
export function sessionDown(): DownReason {
...
export function configureTierless(opts: ConnectOpts & { preconnect?: boolean }): void {
  ...
  (globalThis as { __tierlessBuild?: string }).__tierlessBuild = TIERLESS_BUILD;
