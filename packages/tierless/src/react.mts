...
// The useState value itself — same fields as ActionState minus run/running (those are
// derived, not stored).
...
export function useAction<A extends unknown[], R>(action: (...args: A) => Promise<R> | R): ActionState<A, R> {
  const [state, set] = useState<RunState<R>>({ status: "idle", result: undefined, error: undefined });
  ...
  const run = useCallback(async (...args: A): Promise<R> => {
    ...
    try {
      ...
      if (seq.current === mine) set({ status: "done", result, error: undefined });
      return result;
    } catch (error) {
      if (seq.current === mine) set({ status: "error", result: undefined, error });
      throw error;
    }
  }, [action]);
