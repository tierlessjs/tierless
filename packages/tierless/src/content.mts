...
export class ContentStore {
  private _byHash: Map<string, unknown>;
  private _byObj: Map<unknown, string>;
  constructor() { this._byHash = new Map(); this._byObj = new Map(); }
  register(obj: object): string { const h = hashOf(obj); this._byHash.set(h, obj); this._byObj.set(obj, h); return h; }   // producer: mark a subgraph immutable
  hashFor(obj: object): string | undefined { return this._byObj.get(obj); }                                              // encode: registered? -> its hash, else undefined
  has(h: string): boolean { return this._byHash.has(h); }
  get(h: string): unknown { return this._byHash.get(h); }
  put(h: string, obj: unknown): void { if (!this._byHash.has(h)) { this._byHash.set(h, obj); this._byObj.set(obj, h); } } // receiver: cache a decoded subgraph by hash
}
...
export const newPeerView = (): Set<string> => new Set();
