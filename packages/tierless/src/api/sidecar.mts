...
//
// Why this is the right shape for Tierless: a browser→backend api call is "migrate the continuation one
...
export function serve(api: Api): void {
  ...
  process.on("message", async (msg: any) => {
    ...
    catch (e: any) { res = { ok: false, error: String((e && e.message) || e) }; }  // never crash the monitor on one bad call
...
export function startSidecar(entryUrl: URL | string, env: Record<string, string> = {}): SidecarClient {
  ...
  let nextId = 1;
  const pending = new Map<number, { resolve: (value: HandleResult) => void; reject: (reason: unknown) => void }>();
  let markReady: () => void;
  const readyP: Promise<void> = new Promise((r) => { markReady = r; });
  ...
  return {
    ...
    call: (name: string, args: unknown[] = [], token: string | null = null): Promise<HandleResult> => new Promise((resolve, reject) => {
      const id = nextId++;
      pending.set(id, { resolve, reject });
      child.send({ id, call: { name, args, token } } as any);
    }),
    close: () => child.kill(),
  };
}
...
export function sidecarMain(apiDef: ApiDef | ((secret: string | Uint8Array) => Api), { init }: { init?: () => void } = {}): boolean {
  if (process.env.TIERLESS_SIDECAR !== "1") return false;
  if (init) init();
...
export function makeApiExec(client: SidecarClient | { call: SidecarClient["call"] }, token: string | null = null): (req: { name: string; args: unknown[] }) => Promise<unknown> {
  return async (req) => {
