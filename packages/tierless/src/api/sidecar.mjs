...
export function serve(api) {
    if (!process.send)
        throw new Error("serve(): not running as a forked sidecar (no IPC channel)");
    ...
    process.on("message", async (msg) => {
        ...
        let res;
        try {
            res = await api.handle(msg.call);
        }
        catch (e) {
            res = { ok: false, error: String((e && e.message) || e) };
        } // never crash the monitor on one bad call
...
export function startSidecar(entryUrl, env = {}) {
    ...
    child.on("exit", () => { for (const p of pending.values())
        p.reject(new Error("sidecar exited")); pending.clear(); });
    return {
        ready: () => readyP,
...
export function sidecarMain(apiDef, { init } = {}) {
    if (process.env.TIERLESS_SIDECAR !== "1")
        return false;
    if (init)
        init();
...
export function makeApiExec(client, token = null) {
    return async (req) => {
        if (!req.name.startsWith("api."))
            throw new Error("backend client can't service " + req.name);
        const res = await client.call(req.name.slice(4), req.args, token); // "api.getTasks" → monitor fn "getTasks"
        if (!res.ok)
            throw new Error(res.error); // denial/error → catchable throw across the tier
        return res.value;
    };
}
