...
export class Api {
    ...
    fn(name, def) {
        if (typeof name !== "string" || !name)
            throw new Error("api.fn: name must be a non-empty string");
        if (this._fns.has(name))
            throw new Error(`api.fn(${name}): already registered`);
        if (!def || typeof def.run !== "function")
            throw new Error(`api.fn(${name}): run must be a function`);
        if (!("authorize" in def))
            throw new Error(`api.fn(${name}): authorize is required — every endpoint must state who may call it. Pass ` +
    ...
    async handle(call) {
        ...
        if (!entry)
            return this._deny(name, null, "unknown");
        // Resource budget: reject an oversized payload before doing any crypto or running anything.
        if (this._maxArgsBytes) {
            let n;
            try {
                n = JSON.stringify(args).length;
            }
            catch {
                n = Infinity;
            }
            if (n > this._maxArgsBytes)
                return this._deny(name, null, "oversize");
        }
        let principal = null;
        try {
        ...
        if (this._rate && !this._allowRate((principal && principal.sub) || "anon"))
            return this._deny(name, principal, "ratelimited");
        let allowed = false;
        const { authorize, run } = entry;
        if (authorize === DENY)
            allowed = false;
        else if (authorize === PUBLIC)
            allowed = true;
        // fail closed: the authorizer must return EXACTLY true; a truthy-but-not-true value or a throw denies.
        else {
            try {
                allowed = authorize(principal, args) === true;
            }
            catch {
                allowed = false;
            }
        ...
        if (!allowed)
            return this._deny(name, principal, "unauthorized");
        try {
            const value = await run(args, principal);
            this._log(name, principal, "ok");
            return { ok: true, value };
        }
        catch (e) {
            this._log(name, principal, "error");
    ...
    _allowRate(key) {
        ...
        if (now - this._lastSweep >= win) {
            for (const [k, v] of this._calls)
                if (now - v.start >= win)
                    this._calls.delete(k);
            this._lastSweep = now;
        }
        let s = this._calls.get(key);
        if (!s || now - s.start >= win) {
            s = { start: now, count: 0 };
            this._calls.set(key, s);
        }
        s.count++;
...
export class JwtApi extends Api {
    _secret;
    constructor(secret, opts) { super(opts); if (!secret)
        throw new Error("JwtApi: a signing secret is required"); this._secret = secret; }
    ...
    verify(token) {
        ...
        if (a.length !== b.length || !timingSafeEqual(a, b))
            return null; // signature must match (constant-time)
        let claims;
        try {
            claims = JSON.parse(unb64url(body));
        }
        catch {
            return null;
        }
