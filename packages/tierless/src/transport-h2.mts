...
import { encodeMessage, decodeMessage, type Port } from "./transport.mjs";
...
export function isWebSocketConnect(headers: Record<string, unknown>): boolean {
...
function encodeFrame(payload: Uint8Array, opcode = OP_BIN): Buffer {
  ...
  else { out[1] = 127; out.writeUInt32BE(0, 2); out.writeUInt32BE(n, 6); }   // messages never exceed 4 GiB here
...
function makeParser(onMessage: (data: Uint8Array) => void, onPing: (p: Uint8Array) => void, onClose: () => void): (chunk: Buffer) => void {
  ...
  return (chunk: Buffer): void => {
    ...
    for (;;) {
      ...
      if (masked) { const m = buf.subarray(maskOff, maskOff + 4); const u = Buffer.from(payload); for (let i = 0; i < u.length; i++) u[i] ^= m[i & 3]; payload = u; }
      buf = buf.subarray(off + len);
      if (opcode === OP_PING) { onPing(Uint8Array.from(payload)); continue; }
      if (opcode === OP_PONG) continue;
      if (opcode === OP_CLOSE) { onClose(); return; }
      if (opcode === OP_BIN || opcode === OP_TEXT) { fragOp = fin ? -1 : opcode; if (fin) onMessage(Uint8Array.from(payload)); else frags.push(Uint8Array.from(payload)); }
      else if (opcode === OP_CONT) { frags.push(Uint8Array.from(payload)); if (fin) { const whole = Buffer.concat(frags); frags.length = 0; fragOp = -1; onMessage(Uint8Array.from(whole)); } }
    }
  };
}
...
export function h2Port(stream: ServerHttp2Stream): Port {
  let onMsg: ((obj: unknown, bin: Uint8Array | null) => void) | null = null;
  const feed = makeParser(
    (data) => { if (!onMsg) return; let m; try { m = decodeMessage(data); } catch { try { stream.close(); } catch { /* gone */ } return; } onMsg(m.obj, m.bin); },
    (p) => { try { stream.write(encodeFrame(p, OP_PONG)); } catch { /* gone */ } },
    () => { try { stream.close(); } catch { /* gone */ } },
  );
  stream.on("data", (c: Buffer) => { try { feed(c); } catch { try { stream.close(); } catch { /* gone */ } } });
  return {
    send(obj: object, bin?: Uint8Array): void { try { stream.write(encodeFrame(encodeMessage(obj, bin))); } catch { /* gone */ } },
    onMessage(cb: (obj: unknown, bin: Uint8Array | null) => void): void { onMsg = cb; },
    onClose(cb: () => void): void { stream.on("close", cb); },
    close(): void { try { stream.close(); } catch { /* already gone */ } },
  };
}
