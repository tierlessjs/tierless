...
export function twinHttp(baseUrl: string, { token, headers = {}, fetchImpl = fetch }: { token?: string; headers?: Record<string, string>; fetchImpl?: typeof fetch } = {}): Record<string, unknown> {
  ...
  const call = async (method: string, url: string, data: unknown, config: { params?: Record<string, unknown>; headers?: Record<string, string> } = {}): Promise<TwinResponse> => {
    ...
    if (r.status < 200 || r.status >= 300) {
      const err = new Error("Request failed with status code " + r.status) as Error & { response: TwinResponse; isAxiosError: boolean; code: string; status: number };
      err.response = res; err.isAxiosError = true; err.status = r.status; err.code = r.status >= 500 ? "ERR_BAD_RESPONSE" : "ERR_BAD_REQUEST";
      throw err;
    }
    return res;
  };
...
export function coalesceGets(inner: Exec, paths: Iterable<string>): Exec {
  const allow = new Set([...paths].map((p) => p.split("?")[0]));
  const inflight = new Map<string, Promise<unknown>>();
  return async (req) => {
    const r = req as ResourceRequest;
    if (r.name !== "api.get") return inner(req);                       // idempotent, body-less only
    const [path, , opts] = (r.args ?? []) as [unknown, unknown, { headers?: Record<string, string> }?];
    if (!allow.has(String(path).split("?")[0])) return inner(req);     // not a verified endpoint: never joined
    ...
    if (pending) return { ...(await pending as object) };              // joiner: own copy of the shared result
    const p = Promise.resolve(inner(req));
    inflight.set(key, p);
    try { return { ...(await p as object) }; }
    finally { inflight.delete(key); }                                  // leader clears the slot, success or failure
  };
}
...
export function restResources(baseUrl: string, { token, headers = {}, fetchImpl = fetch, envelopeErrors = false, upstreamIdentity = false, onHeaders }: RestResourcesOpts = {}): Exec {
  ...
  return async (req: ResourceRequest) => {
    ...
    r.headers.forEach((v, k) => { if (k === "content-type" || k === "etag" || k === "cache-control" || k === "expires" || k === "vary" || k === "location" || k.startsWith("x-")) hdrs[k] = v; });
