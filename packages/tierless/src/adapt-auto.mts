...
export function autoSession({ url, gatewayPort, path = WS_PATH, storageKey = "tierlessWsUrl", forceBrowser = [], auth = "auto", cross, awaitClaims, preconnect = true, conditional = true }: AutoSessionOpts = {}): AutoSession {
  ...
  const mergeDeclared = (paths: string[]): void => {
    const g = window as { __tierlessForceBrowser?: ForceBrowserDescriptor[] };
    const list = (g.__tierlessForceBrowser ||= []);
    for (const p of paths) {
      const glob = "**" + p;
      if (!list.some((d) => "glob" in d && d.glob === glob)) list.push({ glob });
    }
  };
  ...
  if (auth !== "none" || preconnect) {
    void sessionHello().then((h) => {
      if (h.forceBrowser?.length) mergeDeclared(h.forceBrowser);
