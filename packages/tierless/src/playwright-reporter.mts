...
export default class TierlessMeasureReporter {
  ...
  onBegin(config: FullConfigLike): void {
    ...
    if (WIRE.length) this.chain = this.chain.then(async () => { this.last = await counters(); });
  }
  onTestEnd(test: TestCaseLike, result: TestResultLike): void {
    if (!OUT) return;
    this.chain = this.chain.then(() => this.record(test, result));
  }
  private async record(test: TestCaseLike, result: TestResultLike): Promise<void> {
    if (!OUT) return;
    ...
    if (WIRE.length && (!this.last || !after)) {
      ...
      for (const k of Object.keys(after)) wire["wire" + k[0].toUpperCase() + k.slice(1)] = after[k] - (this.last![k] || 0);
    }
    if (after) this.last = after;            // CHAIN: this closing snapshot opens the next test
