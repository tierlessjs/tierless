...
export function makePump(bundle, { twins } = {}) {
    ...
    async function service(stack, req, execHere) {
        try {
            stack[stack.length - 1].ret = await execHere(req);
        }
        catch (err) {
    ...
    return async function pump(stack, ownsHere, execHere, incoming = null, sink) {
        ...
        const dispatchDyn = async (stack, r) => {
            ...
            if (isHandle(recv)) {
                ...
                if (twin) {
                    ...
                    return { op: "home", tier: recv.owner, name: "dyn:" + r.member, args: [recv, ...r.args] };
            }
            else {
        ...
        for (;;) {
            ...
            if (home && !ownsHere(home.tier))
                return { done: false, request: home, stack };
            ...
            if (r.op === "return") {
                ...
                if (park) {
                    ...
                    if (ownsHere(park.tier))
                        throw new Error("tierless: dynamic call on a handle owned here has no local meaning: " + park.name);
                    return { done: false, request: park, stack };
                }
            }
            else if (r.op === "throw") {
                stack.pop();
                if (!__unwind(stack, r.value))
                    throw r.value; // uncaught after unwinding all frames
            }
