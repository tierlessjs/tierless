...
function parseDelta(bytes) {
  ...
  const changed = []; { const n = r.count(); for (let i = 0; i < n; i++) {
    ...
    else if (kind === 2) { const owner = S(r.varu()), id = S(r.varu()); const h = { __tierless_handle__: true, owner, id }; if (r.u8()) h.kind = S(r.varu()); changed.push({ sid, kind, handle: h }); }       // §5 handle
