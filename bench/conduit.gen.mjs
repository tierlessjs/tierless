...
export const PROGRAMS = {
  homeFeed(F) {
    while (true) switch (F.pc) {
      ...
      default: throw new RangeError("stackmix: invalid pc " + F.pc + " in homeFeed (forged continuation?)");
    }
  },
  favoritedByFollowed(F) {
    while (true) switch (F.pc) {
      ...
      default: throw new RangeError("stackmix: invalid pc " + F.pc + " in favoritedByFollowed (forged continuation?)");
    }
  },
  drilldown(F) {
    while (true) switch (F.pc) {
...
F.total = F.total + F.art.favoritesCount;
      ...
      case 13:
        F.slug = "article-0";
        F.pc = 12; break;
      default: throw new RangeError("stackmix: invalid pc " + F.pc + " in drilldown (forged continuation?)");
    }
  },
  articlePage(F) {
    while (true) switch (F.pc) {
      ...
      default: throw new RangeError("stackmix: invalid pc " + F.pc + " in articlePage (forged continuation?)");
    }
  }
};
