...
- **Live pump — done (`test/e2e/delta-live.mjs`).** The whole pipeline now runs over a real websocket: the
...
- **§5 excision inside the delta path — done (`encodeDeltaTracked(…, { tier, threshold })`).** The framework's
  ...
  frame, and `adoptBaseline` substitutes too, so ids stay consistent across a full↔delta switch. `test/e2e/delta-live.mjs`
...
- **Write-back IS a delta — landed (`openSnapshot`/`diffSnapshot`/`applySnapshot`).** A write-back and
  ...
  `test/e2e/heap-write-delta.mjs` measures it: member edits in a 1500-row dataset cross at **96×** smaller and
...
- **A larger, framework-shaped sample app — done (`test/e2e/conduit/`).** A RealWorld/Conduit reader
  ...
  compiled continuation — `test/e2e/conduit-verify.mjs` drives an eleven-step user journey headlessly and
...
- **The api as an external reference monitor — landed (`test/e2e/api/`).** The right axis: Stackmix's
  ...
  **api**: a small, stateless reference monitor in its own OS process, reached over a local pipe
  ...
  the floor. `test/e2e/api-verify.mjs` proves it end to end (in `npm test`), including — across a real
...
- **Pump integration — landed (`test/e2e/api-pump.mjs`), with per-call budgets.** A real compiled
